            ...
            <div class="tab-pane{% if loop.first %} active{% endif %} js-gw-tab-pane" id="installment_{{ method }}_">
                <div class="full-width pull-left">
                    ...
                    {% if store.installments_on_steroids_enabled %}
                        ...
                        <div class="{% if method in ['mercadopago'] and store.country == 'AR' %}installments_credit-bank-container p-half-all{% endif %} js-installments-credit-bank-container pull-left full-width m-quarter-top border-box">
                        ...
                        <!-- Cash final info -->
                         <div class="pull-left full-width js-cash-final-info" {% if found_credit_card %}style="display:none;"{% endif %}>
                            ...
                            {% if method in ['boleto_paghiper'] and discount > 0.0 %}
                                <div class="m-half-top m-bottom p-half-bottom"> {{'Boleto Paghiper tiene un' | translate }} <strong class="installment_price-accent">{{discount}}% {{'de descuento' | translate }}</strong> {{'que será aplicado sobre el costo total de la compra al finalizar la misma.' | translate }}</div>
                            {% endif %}
                        </div>
                        <!-- Close button -->
                        <div class="pull-left full-width">
                            <a href="#" class="button installments_close-button js-installments-accept-button pull-right hidden-phone" data-dismiss="modal">{{ 'Cerrar' | translate }}</a>
                            <a href="#" class="button installments_close-button js-installments-accept-button js-mobile-toggle-installments pull-right visible-phone m-half-top-xs" data-dismiss="modal">{{ 'Cerrar' | translate }}</a>
                        </div>
                    {% else %}
                        {% for installment, data_installment in installments %}
