    ...
    <div id="filters-column" class="filters">    
        {% if filter_colors %}
            <div class="filters_container">
                <h4 class="filters_header">{{ 'Color' | translate }}</h4>
                {% for name,color in insta_colors %}
                    <button type="button"
                            class="filter-btn filter-btn-color color-filter"
        ...
        {% if filter_more_colors %}
            <div class="filters_container">
                {% if filter_colors %}
                    <h4 class="filters_header">{{ 'Más colores' | translate }}</h4>
                {% else %}
                    <h4 class="filters_header">{{ 'Color' | translate }}</h4>
                {% endif %}
                {% for color in other_colors %}
                    <button type="button"
                            class="filter-btn size-filter"
        ...
        {% if filter_sizes %}
            <div class="filters_container">
                <h4 class="filters_header">{{ 'Talle' | translate }}</h4>
        ...
        {% for variants_property in variants_properties %}
            {% if filter_other %}
                <div class="filters_container">
                    <h4 class="filters_header">{{ variants_property }}</h4>
