    ...
    <div class="container">
        ...
        <div class="row-fluid visible-when-content-ready">
            <div class="details-box span4">
                ...
                {% if order.address %}
                    <div class="st"><strong>{{'Envío' | translate}}:</strong> 
                    {{ (order.shipping_status == 'fulfilled'? 'Enviado' : 'No enviado') | translate }}
                    </div>

<!-- Código de Rastreio -->                    
<div class="st rastreio"><strong>Código de Rastreio:</strong>&nbsp;{% if order.shipping_tracking_number %}<form id="shipping-correo" method="POST" target="_blank" action="http://www2.correios.com.br/sistemas/rastreamento/resultado_semcontent.cfm"><input type="hidden" name="Objetos" value="{{ order.shipping_tracking_number | raw }}"><a href="javascript:;" onclick="parentNode.submit();">{{ order.shipping_tracking_number | raw }}</a></form>{% endif %}
</div>
<!-- Código de Rastreio -->  
            ...
            <div class="order-box span8" id="orderDetailsDesk">
                ...
                <div class="sst ssb oh">
                    {% if order.pending %}
                        <a href="{{ order.checkout_url | add_param('ref', 'orders_details') }}" target="_blank" class="button p-half-all pull-right">{{ 'Realizar el pago >' | translate }}</a>
                    {% endif %}
                </div>
            </div>
